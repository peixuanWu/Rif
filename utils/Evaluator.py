...
class Evaluator():
    ...
    def MI(cls, image_F, image_A, image_B):
        ...
        return sklearn.metrics.mutual_info_score(image_F.flatten(), image_A.flatten()) + sklearn.metrics.mutual_info_score(image_F.flatten(),
                                                                                                   image_B.flatten())
    ...
    @classmethod
    def fspecial_gaussian(cls, size, sigma):
        """
        Python implementation to mimic MATLAB's fspecial('gaussian', [size, size], sigma)
        """
    ...
    @classmethod
    def ssim_index(cls, img1, img2, K=None, window=None, L=255):
        if K is None:
            K = [0.01, 0.03]

        if window is None:
            window = cls.fspecial_gaussian((11, 11), 1.5)
